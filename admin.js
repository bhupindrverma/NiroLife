...
function renderBoard() {
  ...
  board.innerHTML = visible.length ? visible.map(lead => {
    ...
    const quoteBlock = `<section class="quote-controls"><div><small>Final quotation (₹)</small><input class="quote-amount" data-id="${escapeHtml(lead.id)}" inputmode="numeric" placeholder="9999" value="${escapeHtml(workflow.quoteAmount)}"></div><label><small>Manual payment instructions</small><textarea class="payment-instructions" data-id="${escapeHtml(lead.id)}" rows="2" placeholder="UPI ID or bank-transfer instructions. Do not enter banking passwords.">${escapeHtml(workflow.paymentInstructions)}</textarea></label><button class="send-quote" data-id="${escapeHtml(lead.id)}">Save and email customer</button></section>`;
    ...
    const checklist=workflow.launchChecklist||{}; const launchItems=[['domain','Domain ownership'],['dns','DNS connected'],['ssl','SSL active'],['contact','Phone and WhatsApp'],['forms','Forms tested'],['analytics','Analytics connected'],['approval','Approval recorded']];
...
function bindActions() {
  ...
  document.querySelectorAll('.send-quote').forEach(button => button.addEventListener('click', () => { const id=button.dataset.id; const quoteAmount=document.querySelector(`.quote-amount[data-id="${CSS.escape(id)}"]`).value; const paymentInstructions=document.querySelector(`.payment-instructions[data-id="${CSS.escape(id)}"]`).value; if(!quoteAmount){loginStatus.textContent='Enter the final quotation before emailing the customer.';return;} updateLead(id,{workflowStage:'payment_review',quoteAmount,paymentInstructions,notifyCustomer:true},button); }));
